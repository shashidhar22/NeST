...
//		prime2=-1;
		...
		Timer t=new Timer();
		ArrayList<Sketch> sketches=loadSketches_MT(10000, list);
		t.stop();
...
//	}
	...
	public static final long[] toSketchArray(LongHeap heap, int maxLen){
		final int len=Tools.min(maxLen, heap.size());
...
//	}
	...
	public static ArrayList<Sketch> loadSketches_MT(int maxLen, ArrayList<String> fnames){
		return loadSketches_MT(maxLen, fnames.toArray(new String[0]));
	}
	
	public static ArrayList<Sketch> loadSketches_MT(int maxLen, String...fnames){
		...
		if(decomposedFnames.size()==1){return loadSketches(decomposedFnames.poll());}
		...
		for(int i=0; i<threads; i++){
			allt.add(new LoadThread(decomposedFnames, maxLen));
		}
	...
	private static class LoadThread extends Thread{
		...
		private LoadThread(ConcurrentLinkedQueue<String> queue_){
			this(queue_, 10000);
		}
		
		public LoadThread(ConcurrentLinkedQueue<String> queue_, int maxLen_) {
			queue=queue_;
			maxLen=maxLen_;
...
//		ConcurrentHashMap<Integer, Sketch> map;
	...
	public static ArrayList<Sketch> loadSketches(String fname){
		...
		if(ff.fasta()){
			assert(false) : "Fasta support - TODO.  Please create a sketch first with sketch.sh.";
		}
					...
					if(line.length>1){
						...
						for(String s : split){
							if(s.startsWith("SIZE")){//Sketch length
								...
								size=Integer.parseInt(s.substring(3));
...
//	public static final boolean A48=Sketch.A48;
//	public static final boolean delta=Sketch.delta;
